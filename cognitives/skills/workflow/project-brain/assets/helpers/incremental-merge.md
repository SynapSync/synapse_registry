...
2. **Split** by `## ` (h2 headings) into sections
...
2. Collapse them into a single summary block:
...
3. Place the summary block **at the bottom** of the Session Log (after all individual entries)
4. Remove the individual entries for sessions 1-{M}

This keeps the Session Log focused on recent context while preserving historical knowledge.
...
1. Normalize the item text: lowercase, trim whitespace, remove leading `- `
2. Compare against existing items using the normalized form
3. If a match is found, skip the new item
4. If the new item is similar but not identical (>80% overlap), append it with a note: `(similar to existing)`
